...
OUTPUT_DIR = os.environ.get(
    "INFERENCE_RULE_V9_OUTPUT_DIR",
    _default_path(
        r"/Users/byeol/Desktop/inference/v2/109_sna",
        os.path.expanduser("~/Desktop/inference/inference_rule_v9_v4_out"),
    ),
)

# 전역 탐색용 conf
GLOBAL_CONF_THRES = 0.25
...
W_Y_ALIGN = 1.8
W_CENTER_SYM = 0.9
# 분석에 따라 -0.8 -> 1.8 로 상향 (기존엔 사실상 무필터)
MIN_ACCEPTABLE_PAIR_SCORE = 1.8
...
INIT_FALLBACK_HEIGHT_SIM_MIN = 0.55   # 파트너 height 유사도 최소
INIT_FALLBACK_Y_DIFF_RATIO_MAX = 0.20 # |dy| / frame_height 최대
...
LOCAL_TRIGGER_LOW_CONF = 0.55              # v4: 0.45→0.55, 위태 conf 도 보강
...
def association_score(
    ...
    if side_track.pred_cx is not None and side_track.pred_cy is not None:
        ...
        if side_track.det is not None:
            ...
            # 정지 페널티: 기존 -0.8 -> -0.4 (역도 정지 구간이 길어 과한 페널티 방지)
...
def run_inference_robust(model):
    ...
    log("===== inference start (v4) =====", log_path)
    ...
    try:
        ...
        while True:
            ...
            if track_state.initialized:
                ...
                if run_local:
                    ...
                    if local_roi is not None:
                        ...
                        local_frame = frame[roi_y1:roi_y2, roi_x1:roi_x2]
            ...
            if not track_state.initialized:
                ...
                if left_det is None or right_det is None:
                    emergency_dets = run_emergency_local_inference(
                        model=model, frame=frame, track_state=track_state,
            ...
            if SAVE_VIDEO:
                ...
                plotted_original = global_result.plot()
