...
INPUT_GLOB = os.environ.get(
    ...
    "detections_selected_*.csv",
)
OUTPUT_PREFIX = "detections_postproc_"
...
def update_existing_rows_metadata(L: pd.DataFrame, R: pd.DataFrame):
    ...
    벡터화 버전 — 동작은 기존 row-iter 와 동일하지만 pandas 오버헤드를 제거.
    """
    for df in (L, R):
        if "source" not in df.columns or len(df) == 0:
            continue
        src_col = df["source"].astype("object")
        for src_key, code in INTERP_CODE.items():
            mask = src_col == src_key
            if mask.any():
                df.loc[mask, "is_interpolated"] = code
        if "confidence" in df.columns:
            kp_mask = (src_col == SRC_KALMAN) & df["confidence"].isna()
            if kp_mask.any():
                df.loc[kp_mask, "confidence"] = CONF_PROXY[SRC_KALMAN]
...
def correct_side_swaps(L: pd.DataFrame, R: pd.DataFrame, frames: List[int]) -> List[int]:
    ...
    벡터화: swap 대상 프레임을 boolean mask 로 한 번에 식별 → 컬럼 일괄 교환.
    """
    common_idx = L.index.intersection(R.index)
    if len(common_idx) == 0:
        return []
    lcx = L["center_x"].reindex(common_idx)
    rcx = R["center_x"].reindex(common_idx)
    valid = lcx.notna() & rcx.notna()
    swap_mask = valid & (lcx.astype(float) > rcx.astype(float))
    swap_idx = common_idx[swap_mask.values]
    if len(swap_idx) == 0:
        return []

    swappable_cols = [c for c in L.columns
                      if c not in ("frame_idx", "side", "track_id") and c in R.columns]
    if not swappable_cols:
        return swap_idx.tolist()

    l_block = L.loc[swap_idx, swappable_cols].copy()
    r_block = R.loc[swap_idx, swappable_cols].copy()
    L.loc[swap_idx, swappable_cols] = r_block.values
    R.loc[swap_idx, swappable_cols] = l_block.values
    return swap_idx.tolist()
...
def smooth_trajectory(
    ...
    for df in (L, R):
        # sort_index 결과를 한 번만 캐시하여 4 컬럼 반복에서 재정렬 비용 제거
        df_sorted = df.sort_index()
        sorted_idx = df_sorted.index
        is_interp_sorted = df_sorted["is_interpolated"].to_numpy() \
            if "is_interpolated" in df_sorted.columns else None
        ...
        for col in ("center_x", "center_y", "width", "height"):
            ...
            if mode == "filled_only":
                if is_interp_sorted is None:
                    continue
                smoothed = _savgol_or_ma(vals, SMOOTH_WINDOW, SMOOTH_POLYORDER)
                mask = is_interp_sorted >= 2
                vals_out = vals.copy()
                vals_out[mask] = smoothed[mask]
            else:
                vals_out = _savgol_or_ma(vals, SMOOTH_WINDOW, SMOOTH_POLYORDER)

            # 벡터화 write-back — index 정렬은 sorted_idx 와 동일
            df.loc[sorted_idx, col] = vals_out

        # ===== box 재구성 (벡터화) =====
        cx = df["center_x"].astype(float)
        cy = df["center_y"].astype(float)
        w  = df["width"].astype(float)
        h  = df["height"].astype(float)
        valid = ~(cx.isna() | cy.isna() | w.isna() | h.isna())
        if not valid.any():
            continue
        half_w = w * 0.5
        half_h = h * 0.5
        df.loc[valid, "x1"] = (cx - half_w)[valid]
        df.loc[valid, "y1"] = (cy - half_h)[valid]
        df.loc[valid, "x2"] = (cx + half_w)[valid]
        df.loc[valid, "y2"] = (cy + half_h)[valid]
        area = w * h
        df.loc[valid, "area"] = area[valid]
        if frame_area_total is not None and frame_area_total > 0:
            df.loc[valid, "area_ratio"] = (area / frame_area_total)[valid]
        valid_h = valid & (h > 1e-6)
        if valid_h.any():
            df.loc[valid_h, "aspect_ratio"] = (w / h)[valid_h]
...
def output_path_for(in_csv: str) -> str:
    ...
    suffix = base.replace("detections_selected_", "").replace(".csv", "")
