...
# =========================
# Fast mode (selected_csv 만 산출 — Mac MPS 환경에서 시간 최적화)
# =========================
# True 면 SAVE_VIDEO / raw_csv / pairs_csv 단계를 건너뛴다. 동작/정확도는 동일.
FAST_MODE_SELECTED_ONLY = os.environ.get(
    "INFERENCE_MOTION_V2_FAST", "1"
).lower() not in ("0", "false", "no", "off")

SAVE_VIDEO = False if FAST_MODE_SELECTED_ONLY else True
...
def build_output_paths(output_dir: str, global_conf: float, local_conf: float):
    ...
    return {
        ...
        "selected_csv_path": os.path.join(output_dir, f"detections_selected_{clip}.csv"),
...
_MORPH_KERNEL_CACHE: dict = {}


def _get_morph_ellipse_kernel(px: int) -> np.ndarray:
    k = _MORPH_KERNEL_CACHE.get(px)
    if k is None:
        k = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (px * 2 + 1, px * 2 + 1))
        _MORPH_KERNEL_CACHE[px] = k
    return k
...
def motion_based_darkening(frame: np.ndarray, median_bg: np.ndarray,
    ...
    # cv2.absdiff: uint8 도메인에서 |a-b| 가 항상 안전 (<=255). SIMD 가속.
    diff = cv2.absdiff(frame, median_bg)                     # (H,W,3) uint8
    diff_max = diff.max(axis=2).astype(np.float32)           # (H,W) float32
    weight = np.clip((diff_max - threshold) / max(soft_range, 1e-6), 0.0, 1.0)

    if dilate_px > 0:
        mask01 = (weight > 0.5).view(np.uint8) * 255         # bool view 후 ×255, copy 1회 절약
        mask01_d = cv2.dilate(mask01, _get_morph_ellipse_kernel(dilate_px),
                              iterations=1).astype(np.float32) / 255.0
        weight = np.maximum(weight, mask01_d)

    blended_alpha = alpha_bg + (1.0 - alpha_bg) * weight      # (H,W) float32, 범위 [alpha_bg, 1.0]
    # blended_alpha ∈ [alpha_bg, 1] 이고 frame ∈ [0,255] → out ∈ [0,255] 이 수학적으로 보장됨.
    # 기존 np.clip(out, 0, 255) 는 redundant 라서 25MB write pass 1번 제거.
    out = frame.astype(np.float32, copy=True)
    out *= blended_alpha[..., None]
    return out.astype(np.uint8), weight
...
def extract_detections_from_boxes(
    ...
    # MPS / CUDA 텐서를 한 번에 CPU로 — box 당 .item()/.tolist() 의 동기화 비용 제거
    xyxy_arr = boxes.xyxy.detach().cpu().numpy().astype(np.float64, copy=False)
    if boxes.conf is not None:
        conf_arr = boxes.conf.detach().cpu().numpy().astype(np.float64, copy=False)
    else:
        conf_arr = None
    if boxes.cls is not None:
        cls_arr = boxes.cls.detach().cpu().numpy().astype(np.int64, copy=False)
    else:
        cls_arr = None

    # 벡터화된 clip + offset (원본 clip_box 의 동작과 비트 동일)
    local_w_lim = float(max(local_width - 1, 0))
    local_h_lim = float(max(local_height - 1, 0))
    w_lim = float(max(width - 1, 0))
    h_lim = float(max(height - 1, 0))

    x1 = np.clip(xyxy_arr[:, 0], 0.0, local_w_lim) + x_offset
    y1 = np.clip(xyxy_arr[:, 1], 0.0, local_h_lim) + y_offset
    x2 = np.clip(xyxy_arr[:, 2], 0.0, local_w_lim) + x_offset
    y2 = np.clip(xyxy_arr[:, 3], 0.0, local_h_lim) + y_offset
    np.clip(x1, 0.0, w_lim, out=x1)
    np.clip(y1, 0.0, h_lim, out=y1)
    np.clip(x2, 0.0, w_lim, out=x2)
    np.clip(y2, 0.0, h_lim, out=y2)

    w_arr = x2 - x1
    h_arr = y2 - y1
    cx_arr = (x1 + x2) * 0.5
    cy_arr = (y1 + y2) * 0.5
    area_arr = w_arr * h_arr

    time_sec_val = frame_idx / fps if fps > 0 else None
    frame_wh = max(width * height, 1)
    names = model.names
    ...
    for i in range(n):
        ...
        row = {
            ...
            "width": w_i,
            "height": h_i,
            "center_x": float(cx_arr[i]),
            "center_y": float(cy_arr[i]),
            "area": area_i,
            "area_ratio": area_i / frame_wh,
            "aspect_ratio": w_i / max(h_i, 1e-6),
...
class ChunkedCsvWriter:
    ...
    enabled=False 일 때 모든 메서드는 no-op — 출력 파일 자체를 만들지 않는다.
    """
    def __init__(self, path: str, columns: List[str],
                 flush_every: int = CSV_FLUSH_EVERY_FRAMES, enabled: bool = True):
...
def run_inference_robust(model):
    ...
    writer = None
    rule_writer = None
    # FAST_MODE_SELECTED_ONLY 일 때 raw / pairs CSV 는 디스크에 쓰지 않는다 (no-op writer).
    raw_csv = ChunkedCsvWriter(
        paths["raw_csv_path"], RAW_COLUMNS,
        enabled=not FAST_MODE_SELECTED_ONLY,
    )
    selected_csv = ChunkedCsvWriter(paths["selected_csv_path"], SELECTED_COLUMNS)
    pairs_csv = ChunkedCsvWriter(
        paths["pairs_csv_path"], PAIR_SCORE_COLUMNS,
        enabled=not FAST_MODE_SELECTED_ONLY,
    )
    ...
    try:
        ...
        # === MPS 그래프 컴파일 warm-up ===
        # 첫 predict 호출은 MPS 백엔드에서 커널 컴파일 비용으로 0.5~1.5s 가 추가된다.
        # 더미 프레임으로 global / local imgsz 두 번 미리 돌려 메인 루프 진입 후 매끄럽게.
        try:
            _warm = np.zeros((min(IMGSZ, 1280), min(IMGSZ, 1280), 3), dtype=np.uint8)
            warm_start = time.perf_counter()
            predict_with_thresholds(
                model=model, frame=_warm,
                conf_thres=GLOBAL_CONF_THRES, iou_thres=IOU_THRES,
                inference_device=inference_device, imgsz=IMGSZ,
            )
            predict_with_thresholds(
                model=model, frame=_warm,
                conf_thres=LOCAL_CONF_THRES, iou_thres=IOU_THRES,
                inference_device=inference_device, imgsz=LOCAL_INFERENCE_IMGSZ,
            )
            sync_inference_device_if_needed(inference_device)
            log(f"[INFO] MPS/device warmup done ({(time.perf_counter()-warm_start)*1000:.1f}ms)",
                log_path)
        except Exception as e:
            log(f"[WARN] warmup skipped: {e}", log_path)
